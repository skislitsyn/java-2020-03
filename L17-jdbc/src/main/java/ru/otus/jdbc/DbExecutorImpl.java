...
public class DbExecutorImpl<T> implements DbExecutor<T> {
	...
	Savepoint savePoint = connection.setSavepoint("executeUpdate");
	try (var pst = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
	    for (int idx = 0; idx < params.size(); idx++) {
		pst.setObject(idx + 1, params.get(idx));
	    }
    ...
    @Override
    public Optional<T> executeSelectAll(Connection connection, String sql, Function<ResultSet, T> rsHandler)
	    throws SQLException {
	try (var pst = connection.prepareStatement(sql)) {
	    try (var rs = pst.executeQuery()) {
		return Optional.ofNullable(rsHandler.apply(rs));
